<div>
    <form wire:submit.prevent="updateDisease">
        @if (!empty($errors))
        @if (count($errors) > 0)
    ...
    <hr>
    <input type="submit" wire:click="updateDisease" value="{{ isset($disease) ? 'Edit Disease' : 'Create Disease' }}" class="btn btn-{{ isset($disease) ? 'warning' : 'primary' }}">
    </form>
</div>
